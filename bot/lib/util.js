...
export const criacaoEnv = async ()=> {
    ...
    "acr_access_key=ebd8b67851e90f2b1f0a5fe191534cd2\n"+
    "acr_access_secret=345rVwhHQbge6DbQxMwoyubklETIgISRep5GJndQ\n\n"+
    "# DEEPGRAM - Coloque abaixo sua chave do DEEPGRAM (Transcrição de aúdio para texto)\n"+
    "dg_secret_key=3cbb235afa8f0bb8872f64155578a38f56845c88\n\n";
...
export const verificarEnv = async ()=> {
    try {
      let resposta = {
          acrcloud: {
            resposta: (process.env.acr_host?.trim() == "identify-us-west-2.acrcloud.com" || 
                       process.env.acr_access_key.trim() == "ebd8b67851e90f2b1f0a5fe191534cd2" || 
                       process.env.acr_access_secret.trim() == "345rVwhHQbge6DbQxMwoyubklETIgISRep5GJndQ")
              ? "A API do ACRCloud ainda não foi configurada corretamente" 
              : "✓ API ACRCloud configurada.",
            cor_resposta: (process.env.acr_host?.trim() == "identify-us-west-2.acrcloud.com" || 
                           process.env.acr_access_key.trim() == "ebd8b67851e90f2b1f0a5fe191534cd2" || 
                           process.env.acr_access_secret.trim() == "345rVwhHQbge6DbQxMwoyubklETIgISRep5GJndQ")
            ? "#d63e3e" : false
          },
          deepgram: {
            resposta: (process.env.dg_secret_key?.trim() == "3cbb235afa8f0bb8872f64155578a38f56845c88") 
              ? "A API do DEEPGRAM ainda não foi configurada" 
              : "✓ API DEEPGRAM configurada.",
            cor_resposta: (process.env.dg_secret_key?.trim() == "3cbb235afa8f0bb8872f64155578a38f56845c88") 
              ? "#d63e3e" : false
          },
        }
